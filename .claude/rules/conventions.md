...
- **Never push unprompted.** Commit freely (only your own files), then stop and report - a push waits for Dor to say "push" / run `/commit-push`. Enforced by `.claude/hooks/guard-push.js`: it reads Dor's last message and blocks `git … push` outright unless he asked for it (when he did, the push runs with no prompt).
