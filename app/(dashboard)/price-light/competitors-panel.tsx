...
// row from `lg` up (they used to wrap 4+1, leaving an orphan card). "table" mode (LiveTickets,
// read from live_events - no network crawl) can't be triggered on demand, since its data
// refreshes overnight from the API, not from a scrape.
...
function CompetitorCard({ row, onDone }: { row: CrawlPanelRow; onDone: () => void }) {
  ...
  // breaker and the reason a table-mode competitor has no button.
  const tip = [
    isTable ? "מתרענן בלילה מה-API, אין סריקה" : `סריקה כל ${row.intervalHours} שעות`,
