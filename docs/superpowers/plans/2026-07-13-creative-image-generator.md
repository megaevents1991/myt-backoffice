...
<div>
  <Label htmlFor="logo_url">
    Logo URL (transparent PNG - creative generator)
  </Label>
  <Input
    id="logo_url"
    value={logoUrl}
  ...
  {logoUrl && !logoUrl.toLowerCase().endsWith(".png") && (
    <p className="text-sm text-destructive mt-1">
      Must be a .png (satori cannot render SVG logos)
    </p>
  )}
</div>
```
...
export function buildPriceText(
  ...
  return mode === "ticket"
    ? `כרטיסים החל מ-${currency}${price}`
    : `חבילות החל מ-${currency}${price}`;
}
    ...
    >
      ...
      <div
        style={{
          ...
          paddingTop: isSquare ? 48 : 28,
        }}
      >
        <div
          style={{
            display: "flex",
            fontSize: isSquare ? 44 : 34,
            fontWeight: 700,
      ...
      <div
        style={{
          ...
          flexDirection: "column",
          alignItems: "center",
          paddingBottom: isSquare ? 56 : 30,
        }}
      >
        <div
          style={{
            display: "flex",
            fontSize: isSquare ? 40 : 30,
            textShadow: "0 2px 6px rgba(0,0,0,0.8)",
          }}
        >
...
export async function GET(req: NextRequest) {
  ...
  if (!homeId || !awayId || !dateText || !price || !(size in SIZES)) {
    return NextResponse.json(
      {
        error:
          "Required: home, away, date, price; optional: time, loc, cur, mode, size",
      },
      { status: 400 },
    );
  }
...
export function CreativeForm({
  ...
  const ready =
    homeId && awayId && homeId !== awayId && dateText && Number(price) > 0;
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Select value={locationText} onValueChange={setLocationText}>
            <SelectTrigger>
              <SelectValue placeholder="Stadium / city" />
            </SelectTrigger>
            <SelectContent>
              {locations.map((l) => (
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1">
            ...
            <Select
              value={mode}
              onValueChange={(v) => setMode(v as "package" | "ticket")}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
        ...
        <div>
          ...
          {eventId && (
            <div className="flex items-center gap-2 mt-2">
              ...
              />
              <Label htmlFor="cg-attach">
                Set as event card image (overwrites current card)
              </Label>
            </div>
          )}
        </div>
      ...
      <div>
        ...
        {previewUrl ? (
          ...
          <img
            ...
            className="w-full max-w-[540px] border rounded-md mt-2"
          />
        ) : (
