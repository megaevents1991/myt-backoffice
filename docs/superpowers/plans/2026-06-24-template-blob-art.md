...
## Task 1: Migration - add art\_\* columns to the 4 template tables
...
## Task 2: Backoffice types - add art\_\* to the template types
...
const [artColorIndex, setArtColorIndex] = useState(
  initial?.art_color_index ?? 0,
);
const [artShapeIndex, setArtShapeIndex] = useState(
  initial?.art_shape_index ?? 0,
);
```
...
## Task 7: myt-main types - carry art\_\* into the runtime shapes
...
## Task 8: myt-main readers - map the art\_\* columns
...
- Produces: readers populate `artImageUrl/artColorIndex/artShapeIndex` (people, blog) so cards can read them. Categories already carry snake*case `art*\*`on the raw`Category` row.
