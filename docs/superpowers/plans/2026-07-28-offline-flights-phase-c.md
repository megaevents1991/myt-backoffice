...
Tell Dor: *"Migration C is committed — it adds one nullable column to `events`. Run `npm run db:push` or the 'Apply DB Migrations' Action."*
...
export async function getLockableFlights(
  eventId: number,
): Promise<Array<{ id: number; label: string; allocated_seats: number | null }>> {
  await requireStaff();
  ...
  const allocated = new Map<number, number>(
    (allocations ?? []).map((a: { flight_id: number; allocated_seats: number }) => [
      a.flight_id,
      a.allocated_seats,
    ]),
  );
...
When `event.locked_flight_id` is set, render an amber banner at the top of the flights card: *"Locked package — customers see only flight #{id}. No Amadeus search. Dates fixed to {def_date_depart} → {def_date_return}."* Include a note that the lock only takes effect on the site once the main-app release is live.
