...
export async function getPortalStats(): Promise<PortalStats> {
  ...
  const totalSalesUsd = paid.reduce(
  ...
  return ((data ?? []) as Row[]).map((r) => {
    ...
    if (info && typeof info === "object") {
      if (Array.isArray((info as { events?: unknown }).events)) {
        const first = (
          info as { events: { name?: string; event_name?: string }[] }
        ).events[0];
...
- [ ] Forbidden columns never selected (grep the portal actions for final*purchase_price_ils/payment_info/accounting_number/offline*\*\_cost/exchange_rate - zero hits)
