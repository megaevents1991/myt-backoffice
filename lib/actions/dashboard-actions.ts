...
export async function getDashboardCounts() {
  ...
  const [eventsCount, partnerSplit, paidReservationsCount, pendingReservationsCount] =
    await Promise.all([
      (async () => {
        try {
          const { count, error } = await supabase
...
export async function getDashboardStats() {
  ...
  try {
    ...
    const [paidRes, partnersRes, recentRes, lastMonthRes, last7Res, currentMonthRes] =
      await Promise.all([
        supabase
          .from("reservations")
    ...
    const paidReservations = (paidRes.data || []) as unknown as ReservationRow[]
    ...
    const topPartnerCommission = partners.length > 0 ? Number(partners[0].commission) || 0 : 0
    ...
    const lastMonthRows = (lastMonthRes.data || []) as unknown as ReservationRow[]
    ...
    const currentMonthRows = (currentMonthRes.data || []) as unknown as ReservationRow[]
