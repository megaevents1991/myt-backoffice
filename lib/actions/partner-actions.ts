    ...
    switch (column) {
      ...
      case "commission_type": {
        ...
        if (!COMMISSION_TYPES.includes(commissionType)) {
          throw new Error(
            `commission_type must be one of ${COMMISSION_TYPES.join(", ")}`,
          );
        }
      ...
      case "supplier_number":
        row[column] =
          value === "" || value == null
            ? null
            : assertMoney("supplier_number", value);
        break;
 ...
 */
export async function getCustomerRefundPartners(
  limit = 200,
): Promise<CustomerRefundPartners> {
  await requireStaff();
  ...
  if (countResult.error) {
    console.error(
      "getCustomerRefundPartners count:",
...
export async function createPartner(
  partner: PartnerInput & { partner_tracking_code: string },
) {
  await requireStaff();
 ...
 */
async function deletePortalLogins(
...
export async function setPartnersActive(
  ...
  if (profileError) {
    ...
    return {
      ok: false,
      error:
        "Partner status saved, but the portal login state could not be synced",
    };
  }
...
async function generateCopyTrackingCode(trackingCode: string): Promise<string> {
  ...
  const taken = new Set(
    (data ?? []).map(
      (r) => (r as { partner_tracking_code: string }).partner_tracking_code,
    ),
  );
