...
async function assertNoCycle(id: number, parentId: number | null): Promise<void> {
  ...
  const { data, error } = await db().select("id,parent_id").eq("is_deleted", false);
...
async function categoryPath(id: number): Promise<string> {
  const { data, error } = await db().select("id,parent_id,slug").eq("is_deleted", false);
  if (error) throw error;
...
async function syncLink(id: number): Promise<void> {
  ...
  const replaceable = link === "" || link.startsWith("/c/") || link.startsWith("/category/");
...
export async function updateCategory(
  ...
  await requireStaff();
  if (data.parent_id !== undefined) await assertNoCycle(id, data.parent_id ?? null);
