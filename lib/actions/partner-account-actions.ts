...
function validate(input: PartnerAccountInput): string | null {
  ...
  if (
    !Number.isFinite(input.credit_per_ticket) ||
    input.credit_per_ticket < 0
  ) {
  ...
  if (
    input.supplier_number != null &&
    !Number.isFinite(input.supplier_number)
  ) {
...
export async function updatePartnerAccount(
  ...
  if (
    current &&
    isCustomerRefundPartner(
      current as { type?: string | null; name_hebrew?: string | null },
    )
  ) {
    return {
      ok: false,
      error:
        "This is an automatic customer-refund record and cannot be edited as a partner",
    };
  }
  ...
  if (current) {
    const frozen = await freezeCommissionOnExistingReservations({
      trackingCode,
      previousType:
        (current as { commission_type?: string | null }).commission_type ??
        null,
      previousRate:
        (current as { commission?: number | null }).commission ?? null,
  ...
  if (profileError) {
    console.error(
      "updatePartnerAccount profile:",
      JSON.stringify(profileError),
    );
    return {
      ok: false,
      error: "Partner saved, but its login could not be updated",
    };
  }
  ...
  if (input.password) {
    ...
    if (pwError) {
      ...
      return {
        ok: false,
        error: "Partner saved, but the password could not be changed",
      };
    }
    await logAudit({
...
async function createPortalUser(args: {
  ...
  if (authError || !created?.user) {
    ...
    return {
      ok: false,
      error:
        authError?.message ?? "Could not create the login for this partner",
    };
  }
  ...
  if (profileError) {
    ...
    await supabase.auth.admin
      ...
      .catch((e) =>
        console.error(
          "createPortalUser rollback failed (orphan auth user):",
          JSON.stringify(e),
        ),
      );
