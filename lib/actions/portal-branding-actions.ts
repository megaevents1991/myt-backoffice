...
export type BrandingResult = { ok: true; logo_url: string } | { ok: false; error: string }

export async function uploadPartnerLogo(formData: FormData): Promise<BrandingResult> {
...
export async function removePartnerLogo(): Promise<{ ok: true } | { ok: false; error: string }> {
  const session = await requirePartner()
