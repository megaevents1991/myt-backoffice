...
export async function createCoupon(coupon: CouponInput) {
  ...
  await logAudit({ action: "create", entityType: "coupon", entityId: created.id, changes: payload })
  return created
}
...
export async function updateCoupon(id: number, coupon: Partial<CouponInput>) {
  ...
  if (error) throw error
  await logAudit({ action: "update", entityType: "coupon", entityId: id, changes: diffChanges(before, patch) })
