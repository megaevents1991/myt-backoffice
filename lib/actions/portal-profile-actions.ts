...
export async function getMyProfileDetails(): Promise<MyProfileDetails | null> {
  ...
  if (partnerResult.error?.code === "42703") {
    ...
    partnerResult = await fetchPartner(
      "commission,commission_type,user_discount",
    );
  }

  if (partnerResult.error) {
    console.error(
      "getMyProfileDetails partner:",
  ...
  if (profile.contract_url) {
    ...
    if (signError) {
      console.error(
        "getMyProfileDetails contract sign:",
...
export async function changeMyPassword(input: {
  ...
  if (!input.next || input.next.length < MIN_PASSWORD_LENGTH) {
    return {
      ok: false,
      error: `סיסמה חדשה: לפחות ${MIN_PASSWORD_LENGTH} תווים`,
    };
  }
...
export async function rebalanceMyCommissionSplit(input: {
  ...
  const currentTotal =
    Number(data.commission ?? 0) + Number(data.user_discount ?? 0);
  ...
  if (!frozen.ok) {
    return {
      ok: false,
      error:
        "לא הצלחנו לקבע את התנאים על הזמנות קיימות - היחס לא שונה. נסו שוב.",
    };
  }
