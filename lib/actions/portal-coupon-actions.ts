...
export async function getMyCouponTerms(): Promise<MyCouponTerms | null> {
  ...
  const capRaw = Number(
    (data as { coupon_cap?: number | null }).coupon_cap ?? NaN,
  );
  return {
...
export async function fundedCouponCodesFor(
  trackingCode: string,
): Promise<Set<string>> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return fundedCodeSet(
    ((data ?? []) as { code: string | null }[]).map((row) => row.code),
  );
}
 ...
 */
export async function quoteUpliftsFor(
  trackingCode: string,
  ...
  if (settlingError) {
    if (settlingError.code !== "42703") {
      console.error(
        "quoteUpliftsFor reservations:",
        JSON.stringify(settlingError),
      );
    }
    return byReservation;
  }
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (errorCode === "42703" || errorCode === "PGRST204") {
      ...
      return {
        ok: false,
        error:
          "היכולת הזו תהיה זמינה אחרי עדכון המערכת הקרוב. נסו שוב מאוחר יותר.",
      };
    }
    if (errorCode !== "23505") {
