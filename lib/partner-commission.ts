...
export function countReservationTickets(reservation: ReservationLike): number {
  return normalizeReservationEventOrderInfo(
    reservation.event_order_info,
  ).reduce((sum, event) => sum + (Number(event.number_of_ticket) || 0), 0);
}
...
export function commissionForReservation(
  ...
  const snapshotRate =
    ...
    Number.isFinite(Number(reservation.commission_rate))
      ? Number(reservation.commission_rate)
      : null;
  const rate = snapshotRate ?? terms.rate;
  const type =
    snapshotRate != null
      ? (reservation.commission_type ?? terms.type)
      : terms.type;
