...
async function fetchXS2<T = unknown>(
  path: string,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `❌ XS2Event API error: ${res.status} ${res.statusText}`,
        errorText,
      );
      throw new Error(
        `XS2Event API error: ${res.status} ${res.statusText} - ${errorText}`,
      );
    }
...
async function fetchAllPaginated<T = unknown>(
  ...
  while (hasMoreData && currentPage <= maxPages) {
    try {
      ...
      console.log(`🔍 Raw response structure:`, {
        ...
        keys:
          typeof response === "object" && response ? Object.keys(response) : [],
      ...
      if (Array.isArray(response)) {
        ...
        console.log(
          `📋 Object response: ${pageData.length} items, pagination:`,
          pagination,
        );
        ...
        if (pagination) {
          ...
          hasMoreData =
            ...
            (pagination.total_size &&
              ...
              currentPage * pagination.page_size < pagination.total_size) ||
            false;

          console.log(
            `🔄 Pagination check: hasMoreData=${hasMoreData}, pagination=`,
            pagination,
          );
        } else {
          ...
          hasMoreData = pageData.length === 100;
          console.log(
            `🔄 No pagination metadata, fallback check: hasMoreData=${hasMoreData} (got ${pageData.length} items)`,
          );
        }
      }

      console.log(
        `📊 Page ${currentPage}: ${pageData.length} items, hasMoreData: ${hasMoreData}`,
      );
      ...
      if (pageData.length === 0) {
        ...
        console.log(
          `✅ Added ${pageData.length} items, total so far: ${allData.length}`,
        );
      }
  ...
  console.log(
    `✅ Completed paginated fetch for ${path}: ${allData.length} total items across ${currentPage - 1} pages`,
  );
  return allData;
}
...
export async function syncSports(): Promise<SyncResult> {
  try {
    ...
    const sportsResponse = await fetchAllPaginated("sports", {
      cache: "no-store",
    });
...
export async function syncTournaments(): Promise<SyncResult> {
  try {
    ...
    const tournamentsResponse = await fetchAllPaginated("tournaments", {
      cache: "no-store",
    });
    ...
    if (!Array.isArray(tournaments) || tournaments.length === 0) {
      ...
      return {
        status: "error",
        error: "No tournaments data received",
        count: 0,
      };
    }
    ...
    const filteredTournaments = tournaments
      .map((tournament: any) => ({
        ...
        number_events:
          tournament.number_events || tournament.event_count || null,
...
export async function syncEvents(): Promise<SyncResult> {
  try {
    ...
    const eventsResponse = await fetchAllPaginated(eventsPath, {
      cache: "no-store",
    });
    ...
    const filteredEvents = events
      .map((event: any) => ({
        ...
        min_ticket_price_eur: event.min_ticket_price_eur
          ? parseFloat(event.min_ticket_price_eur) / 100
          : null,
        max_ticket_price_eur: event.max_ticket_price_eur
          ? parseFloat(event.max_ticket_price_eur) / 100
          : null,
        slug: event.slug || null,
        number_of_tickets: event.number_of_tickets
          ? parseInt(event.number_of_tickets)
          : null,
      ...
      .filter((event) => {
        ...
        if (
          ...
          console.log(
            `❌ Skipping invalid event: ${event.event_name || "Unknown"} - missing required fields`,
          );
          return false;
        }
    ...
    console.log(
      `📊 Valid events after filtering: ${filteredEvents.length} out of ${events.length} received`,
    );
...
export async function syncSportsEventsData(
  types?: string[],
): Promise<SyncResults> {
