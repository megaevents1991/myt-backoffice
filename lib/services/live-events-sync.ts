...
const LIVE_API_BASE_URL =
  process.env.NEXT_SECRET_LIVE_API_URL || "https://api.doctorticket.com/api";
...
async function fetchLive<T = unknown>(endpoint: string): Promise<T> {
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      ...
      throw new Error(
        `Request timeout - LIVE API did not respond within ${TIMEOUT_MS}ms`,
      );
    }
...
): "sports_live_event_dynamic" | "music_live_event_dynamic" | null {
...
export async function syncLiveEvents(): Promise<LiveSyncResult> {
  try {
    ...
    if (processedEvents.length > 0) {
      ...
      console.log(
        `🗃️ Upserting ${processedEvents.length} events in ${batches.length} batches (size <= ${BATCH_SIZE}).`,
      );

      for (let i = 0; i < batches.length; i++) {
        const batch = batches[i];
        const { error } = await supabase.from("live_events").upsert(batch, {
          onConflict: "event_id",
    ...
    return {
      ...
      status: "success",
      details: "Batched upsert completed",
    };
  } catch (error) {
