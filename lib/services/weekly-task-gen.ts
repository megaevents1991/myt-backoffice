...
export async function runWeeklyTaskGen(
  ...
  // A new price_light task drops its event out of /price-light's "ממתינים להחלטה" (cached
  // screen) - once per run, not per task. No-op outside a Next request.
  if (!opts.dryRun && summary.created > 0) invalidatePriceLight("rows");
  return summary;
}
